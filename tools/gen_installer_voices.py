...
Source of truth is tools/gen_voice_table.py, as it is for the engine's own
table -- the mode table the v1.12 installer wrote. Regenerate with:
...
import gen_voice_table  # noqa: E402
...
def rule_files(modes_for_language):
    ...
    for mode in modes_for_language:
        for key in ("LanguageFile", "LibraryFile"):
...
def main():
    modes = list(gen_voice_table.modes())
    ...
    out.append("; voices and no others. Included by Infovox23012SAPI.iss.")
    out.append(";")
    ...
    out.append("; deleted and recreated on every run (see Infovox23012SAPI.iss), so the")
