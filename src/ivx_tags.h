...
enum class Kind { Text, Engine, Pause, Bookmark, Voice };
...
constexpr Spec kSpecs[] = {
    ...
    {L"vol", Value::Number, Kind::Engine, effect::kVolume},
...
inline bool changes_voice(const std::wstring& tagged)
{
    std::size_t i = 0;
    while (i < tagged.size()) {
        if (tagged[i] != L'\\') {
        ...
        const std::size_t close = tagged.find(L'\\', i + 1);
        if (close == std::wstring::npos) {
            break;
        }
        std::size_t name_length = 0;
        while (i + 1 + name_length < close && tagged[i + 1 + name_length] != L'=') {
            ++name_length;
        }
        if (detail::same_name(tagged.data() + i + 1, name_length, L"vce")) {
            return true;
        }
        i = close + 1;
    }
