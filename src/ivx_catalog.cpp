...
constexpr char kEngineRoot[] = "Software\\Telia Promotor\\Infovox 230\\1.1";
constexpr char kModesRoot[] = "Software\\Telia Promotor\\Infovox 230\\1.1\\Modes";
...
std::wstring local_appdata_ini()
{
    ...
    return std::wstring(base) + L"\\Infovox23012SAPI\\voices.ini";
}
...
void Catalog::seed_virtual_registry(const std::string& engine_dir) const
{
    ...
    // The engine carries its own tracing, which it writes to IVX230_<pid>*.LOG
    // in LogStartupDir. It is the only view there is of what the engine believes
    // about itself, and it is what the reverse engineering was done with -- but
    // it costs a file write per internal call, so it is off unless asked for.
    // INFOVOX23012_ENGINE_LOG=1 in the worker's environment asks.
    //
    // The value has to be the literal "on"; the engine compares the string, and
    // any other value -- including "1" -- reads as off.
    char want_engine_log[16] = {0};
    if (GetEnvironmentVariableA("INFOVOX23012_ENGINE_LOG", want_engine_log,
                                sizeof(want_engine_log)) > 0 &&
        want_engine_log[0] != '0') {
        vr.set_string(kEngineRoot, "LogStartup", "on");
        IVX_INFO("vreg: the engine's own logging is on; it writes IVX230_*.LOG into %s",
                 engine_dir.c_str());
    }
    ...
    // A value that is not set and a value set to the empty string are not the
    // same thing to this engine. Its own installer wrote only the values a mode
    // actually had, and a mode that names, say, a PhSymFile of "" is a mode that
    // has been told to load a file called nothing: it enumerates, it selects
    // without complaint, and it speaks silence. So the four data-file names are
    // written only when there is a name to write.
    //
    // ModeGUID, LanguageID, LanguageFile, SpeakerName and the four synthesis
    // parameters are always written -- every mode has them, and leaving one out
    // is a mode the engine drops.
    for (const Voice& v : voices_) {
        const std::string path = std::string(kModesRoot) + "\\" + v.mode_key;
        auto set_if_named = [&](const char* name, const std::string& value) {
            if (!value.empty()) {
                vr.set_string(path, name, value);
            }
        };
        ...
        set_if_named("LibraryFile", v.library_file);
        set_if_named("PhSymFile", v.phsym_file);
        vr.set_string(path, "SpeakerName", v.speaker_name);
        set_if_named("SpeakerStyle", v.speaker_style);
