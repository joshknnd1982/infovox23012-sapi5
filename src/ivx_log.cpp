...
constexpr wchar_t kLogMutexName[] = L"Local\\Infovox23012SAPI_Log";
...
bool data_dir(wchar_t* out, size_t count)
{
    ...
    if (_snwprintf_s(out, count, _TRUNCATE, L"%s\\Infovox23012SAPI", base) < 0) {
        return false;
    }
...
{
    wchar_t buf[32];
    if (GetEnvironmentVariableW(L"INFOVOX23012_LOG_LEVEL", buf, 32) > 0) {
...
void log_init(const char* module_tag)
{
    ...
    if (data_dir(s.dir, MAX_PATH)) {
        _snwprintf_s(s.path, MAX_PATH, _TRUNCATE, L"%s\\infovox23012.log", s.dir);
