...
{
    // The v1.12 engine reads its configuration from HKEY_LOCAL_MACHINE -- its
    // installer was a machine-wide one -- while the lexicon browser built into
    // the same dll uses HKEY_CURRENT_USER. Both are served the one tree: there
    // is only one configuration, and which hive the engine happens to ask for
    // is not information anybody needs.
    if (key == HKEY_LOCAL_MACHINE || key == HKEY_CURRENT_USER) {
        return &root_;
    }
    if (is_predefined(key)) {
        // HKCR, HKU, HKCC: deliberately empty. The engine finding nothing here
