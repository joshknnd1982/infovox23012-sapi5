...
// %LOCALAPPDATA%\Infovox23012SAPI\voices.ini for one user -- read in that order,
...
struct EngineSettings {
    ...
    // The engine also begins every utterance with about a tenth of a second of
    // the same inaudible dither, and it does so for every piece an utterance is
    // split into -- so a sentence with a pause in the middle gets two of them.
    // A tenth of a second is what a screen reader user waits after every
    // keypress before hearing anything, which is more than the engine itself
    // now costs, so it is dropped as well.
