...
std::wstring find_engine_dir()
{
    ...
    if (GetEnvironmentVariableW(L"INFOVOX23012_ENGINE_DIR", env, MAX_PATH) > 0) {
        if (has_engine(env)) {
            IVX_INFO("paths: engine folder from INFOVOX23012_ENGINE_DIR: %S", env);
            return env;
        }
        IVX_WARN("paths: INFOVOX23012_ENGINE_DIR=%S has no Ivx230nt.dll; ignoring it", env);
    }
    ...
    const std::wstring candidates[] = {
        ...
        parent_of(app) + L"\\bin\\engine",
        parent_of(parent_of(app)) + L"\\bin\\engine",
        parent_of(parent_of(parent_of(app))) + L"\\bin\\engine",
        parent_of(parent_of(parent_of(parent_of(app)))) + L"\\bin\\engine",
    };
...
{
    return install_dir() + L"\\Infovox23012Server.exe";
}
...
std::wstring user_data_dir()
{
    ...
    std::wstring dir = std::wstring(base) + L"\\Infovox23012SAPI";
