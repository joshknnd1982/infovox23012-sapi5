; Inno Setup script for the Infovox 230 v1.12 SAPI5 voices.
;
...
[Setup]
AppId={{A7B09B34-B65A-4E98-9E76-30A115BC20C8}
AppName={#AppName}
...
OutputDir={#StageDir}
OutputBaseFilename=Infovox23012SAPI_Setup_{#AppVersion}
...
UninstallDisplayIcon={app}\Infovox23012Diag.exe
...
; so is the default, because most people want all of it and the twelve
; languages together are 1.6 MB.
...
Source: "{#StageDir}\Infovox23012SAPI.dll";     DestDir: "{app}"; Components: core; Flags: ignoreversion restartreplace uninsrestartdelete
Source: "{#StageDir}\Infovox23012Server.exe";   DestDir: "{app}"; Components: core; Flags: ignoreversion restartreplace uninsrestartdelete
Source: "{#StageDir}\Infovox23012Diag.exe";     DestDir: "{app}"; Components: core; Flags: ignoreversion restartreplace uninsrestartdelete
Source: "{#StageDir}\Infovox23012SapiTest.exe"; DestDir: "{app}"; Components: core; Flags: ignoreversion restartreplace uninsrestartdelete

; --- the configuration utility: define your own voices, and set everything the
;     engine can be told, without editing a file by hand
Source: "{#StageDir}\Infovox23012Config.exe";   DestDir: "{app}"; Components: core; Flags: ignoreversion restartreplace uninsrestartdelete
...
Source: "{#StageDir}\x64\Infovox23012SAPI.dll";     DestDir: "{app}\x64"; Components: core; Flags: ignoreversion restartreplace uninsrestartdelete; Check: Is64BitInstallMode
Source: "{#StageDir}\x64\Infovox23012Diag.exe";     DestDir: "{app}\x64"; Components: core; Flags: ignoreversion restartreplace uninsrestartdelete; Check: Is64BitInstallMode
Source: "{#StageDir}\x64\Infovox23012SapiTest.exe"; DestDir: "{app}\x64"; Components: core; Flags: ignoreversion restartreplace uninsrestartdelete; Check: Is64BitInstallMode
...
Name: "desktopicon"; \
    Description: "Put an &icon for the Infovox 230 v1.12 configuration utility on the desktop"; \
...
begin
  ...
  // memo line by line.
  SummaryMemo.Lines.Add('Infovox 230 v1.12 SAPI5 Voices');
  ...
  SummaryMemo.Lines.Add('carries its own pronunciation rules, from 17 KB for Spanish');
  SummaryMemo.Lines.Add('to 334 KB for British English, and all 12 together come to');
  SummaryMemo.Lines.Add('about 1.6 MB. The whole product is under 5 MB installed.');
  SummaryMemo.Lines.Add('');
  ...
  SummaryMemo.Lines.Add('background program, Infovox23012Server.exe. It starts by itself');
  ...
  SummaryMemo.Lines.Add(ExpandConstant('  {localappdata}\Infovox23012SAPI\infovox230.log'));
  SummaryMemo.Lines.Add(ExpandConstant('  {localappdata}\Infovox23012SAPI\install.log'));
  SummaryMemo.Lines.Add('');
...
begin
  ...
  Exec(ExpandConstant('{sys}\taskkill.exe'), '/F /IM Infovox23012Server.exe',
  ...
  Exec(ExpandConstant('{sys}\taskkill.exe'), '/F /IM Infovox23012Config.exe',
       '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
  Sleep(500);
end;
...
begin
  ...
  begin
    ...
    RegisterServer(ExpandConstant('{app}\Infovox23012SAPI.dll'), False, True);
    if Is64BitInstallMode then
      RegisterServer(ExpandConstant('{app}\x64\Infovox23012SAPI.dll'), True, True);
  end
  ...
  begin
    ...
    RegistrationFailed := '';
    if not RegisterServer(ExpandConstant('{app}\Infovox23012SAPI.dll'), False, False) then
      RegistrationFailed := '32-bit';

    if Is64BitInstallMode then
    begin
      if not RegisterServer(ExpandConstant('{app}\x64\Infovox23012SAPI.dll'), True, False) then
      begin
    ...
    if RegistrationFailed <> '' then
      MsgBox('The ' + RegistrationFailed + ' speech interface could not be registered.' +
             ...
             ExpandConstant('{localappdata}\Infovox23012SAPI\install.log'),
             mbError, MB_OK);
  end;
end;
...
var
  LogFolder: String;
begin
  LogFolder := ExpandConstant('{localappdata}\Infovox23012SAPI');
...
Filename: "{app}\Infovox23012SapiTest.exe"; Parameters: "say"; \
...
Filename: "{app}\Infovox23012Config.exe"; \
    Description: "Open the Infovox 230 v1.12 configuration utility to make voices of my own"; \
