// Infovox23012Config -- the configuration utility.
//
// It exists so that defining a voice does not mean editing an ini file by hand.
// Everything the Infovox 230 v1.12 engine can be told, per voice and for the engine
...
// in %LOCALAPPDATA%\Infovox23012SAPI for one user. The utility edits it through
...
const wchar_t kPreviewSection[] = L"Infovox 230 v1.12 preview (temporary)";
...
{
    MessageBoxW(owner, text.c_str(), L"Infovox 230 v1.12 Configuration", MB_OK | MB_ICONEXCLAMATION);
}
...
void update_details(HWND dlg)
{
    ...
    for (int key = KEY_SPEAKER_STYLE; key <= KEY_PHSYM_FILE; ++key) {
...
{
    const std::wstring dll32 = install_dir() + L"\\Infovox23012SAPI.dll";
    const std::wstring dll64 = install_dir() + L"\\x64\\Infovox23012SAPI.dll";
    if (!file_exists(dll32)) {
        say_problem(dlg, L"The speech interface was not found next to this utility, so the "
                         L"voices cannot be published. Reinstall Infovox 230 v1.12.");
        return false;
    }
...
bool confirm_close(HWND dlg)
{
    if (g_file.dirty()) {
        const int answer =
            MessageBoxW(dlg, L"Save your changes before closing?", L"Infovox 230 v1.12 Configuration",
    ...
    if (g_publish_pending) {
        const int answer = MessageBoxW(
            ...
            L"Infovox 230 v1.12 Configuration", MB_YESNO | MB_ICONQUESTION);
        if (answer == IDYES) {
            publish_voices(dlg);
        }
    }
...
void on_delete(HWND dlg)
{
    ...
    if (MessageBoxW(dlg, question.c_str(), L"Infovox 230 v1.12 Configuration",
...
void on_scope_changed(HWND dlg)
{
    ...
    if (g_file.dirty()) {
        const int answer = MessageBoxW(
            ...
            L"Infovox 230 v1.12 Configuration", MB_YESNOCANCEL | MB_ICONQUESTION);
        if (answer == IDCANCEL) {
    ...
    if (wanted == SCOPE_MACHINE && !scope_is_writable(SCOPE_MACHINE)) {
        MessageBoxW(dlg,
                    ...
                    L"administrator\".",
                    L"Infovox 230 v1.12 Configuration", MB_OK | MB_ICONINFORMATION);
    }
